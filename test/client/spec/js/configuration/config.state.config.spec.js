    ...
    describe('ConfigStateConfig', function () {
        ...
        describe('when initializing', function () {
            it('should do the needful', function () {
                ...
                expect($stateProvider.state.calls.argsFor(1)[0].name).toBe('configuration.role');
                expect($stateProvider.state.calls.argsFor(2)[0].name).toBe('configuration.user');
                expect($stateProvider.state.calls.argsFor(3)[0].name).toBe('configuration.report');
            });
        });
    });
})();
